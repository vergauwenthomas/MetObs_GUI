...
def setup_triggers(MW):
    ...
    # analysing triggers
    MW.print_dataset_info.clicked.connect(lambda: _react_show_dataset_info(MW))

# =============================================================================
# Reactions
# =============================================================================
def _react_conv_outliers_to_gaps(MW):
    pretty_formatter.set_prompt_subsection(f' ---- Converting all outliers to gaps --------',
                                           MW.prompt_fill)
    
    if MW.Dataset.outliersdf.empty:
        Notification('There are no outliers found!')
        return
    
    _, succes, stout = tlk_scripts.gui_wrap(MW.Dataset.convert_outliers_to_gaps,
                                            {})
    if not succes:
        Error('Error in converting outliers to gaps', stout)
        return
    MW.prompt_fill.appendPlainText(str(stout))
    Notification('Outliers are converted to gaps.')
...
def _react_show_dataset_info(MW):
    ...
    MW.prompt_fill.insertPlainText(str(stout))
    if not succes:
        Error(f'.get_info() error on {MW.Dataset}', stout)
        
    MW.prompt_fill.centerCursor()
