...
from metobs_gui.extra_windows import DataDialog
...
def _react_show_modeldata(MW):
    ...
    #get modeldata in dataframe
    # Model = MW.Dataset.gee_datasets[modelname].modeldf
...
def _react_extract_gee_data(MW):
    ...
    MW.prompt_modeldata.appendPlainText(stout)
...
def _react_input_method_change(MW):
    ...
    if method == 'GEE - API':
        ...
        for widg in gee_widgets:widg.setEnabled(False)
        for widg in csv_widgets:widg.setEnabled(False)
        for widg in pkl_widgets: widg.setEnabled(True)
    else:
